...
if USE_SUPABASE:
    try:
        ...
        print(f'⚠️  Supabase connection failed: {_sb_err} — falling back to SQLite')
        USE_SUPABASE = False
...
<div class="box">
  ...
  <div class="badges">
    ...
    <span class="badge {% if ik_ok %}ok{% else %}warn{% endif %}">
      {% if ik_ok %}☁️ ImageKit{% else %}💾 Local uploads{% endif %}
    </span>
  </div>
  <form method="POST">
...
<main class="main">
  <div class="topbar">
    ...
    <div class="tr">
      ...
      <span class="pill {% if ik_ok %}ok{% else %}warn{% endif %}">
        {% if ik_ok %}☁️ ImageKit{% else %}💾 Local{% endif %}
      </span>
      <span class="pill">👤 {{admin_user}}</span>
  ...
  <div class="sec" id="sec-uploads">
    ...
    <div class="storage-info {% if not ik_ok %}warn{% endif %}">
      {% if ik_ok %}
        <strong>☁️ ImageKit connected</strong> — files are uploaded to the ImageKit CDN and served globally.
      {% else %}
        <strong>⚠️ ImageKit not configured</strong> — files save locally (lost on Render restart).
  ...
  <div class="sec" id="sec-settings">
    ...
    <div class="cp">
      ...
      <button class="btn bp" style="min-width:180px" onclick="saveSettings()">💾 Save Settings</button>
      <span id="settingsSaveMsg" style="margin-left:12px;font-size:.8rem;color:#22c55e;display:none">✅ Saved!</span>
    </div>
    ...
    <div class="cp" style="margin-top:20px">
      ...
      <button class="btn bp" style="min-width:180px" onclick="saveBrandSettings()">💾 Save Brand</button>
      <span id="brandSaveMsg" style="margin-left:12px;font-size:.8rem;color:#22c55e;display:none">✅ Saved!</span>
    </div>
  </div>
...
async function uploadFile(file){
  ...
  try{
    ...
    bar.style.width='100%';
    if(d.ok){
      stat.textContent='✅ Uploaded via '+d.storage+'!';
      ...
      res.innerHTML=`<div style="background:var(--bg2);border:1px solid var(--border);border-radius:11px;padding:14px;margin-top:8px">
        <p style="font-size:.78rem;color:var(--muted);margin-bottom:6px">
          Storage: <strong style="color:var(--gold)">${d.storage==='imagekit'?'☁️ ImageKit CDN':'💾 Local'}</strong>
        </p>
...
if __name__ == '__main__':
    ...
    print(f'🗄️  Database    →  {"Supabase" if USE_SUPABASE else "SQLite (local)"}')
    print(f'{"☁️ " if USE_IMAGEKIT else "💾 "} Storage     →  {"ImageKit CDN" if USE_IMAGEKIT else "Local uploads"}')
    print('━' * 56)
